...
- AI 生成旅行计划：`/trips/new` 默认引导到 `/trips/ai-plan`，AI 草稿确认后写入正式旅行数据。
- 旅行管理：目的地、地点、美食、住宿、每日行程、路线、今日模式、预算、清单、笔记、导入和导出。
...
- PWA 与移动端：manifest、Service Worker、离线旅行摘要、网络状态提示、图片压缩、暗色模式和移动端 E2E 覆盖。
