...
- 登录失败按用户名和 IP 独立限流，错误提示不泄露用户名是否存在。
- 修改密码后会清理其他会话。
- `/settings/sessions` 可查看有效会话并退出其他会话。
...
- `DOCUMENT_ENCRYPTION_KEY` 用于上传文件 AES-256-GCM 加密，不能提交、打印或返回前端；丢失后无法解密历史文件。
- 不在日志中输出密码、session token、API Key 或 secret 原文。
- 环境变量校验错误只显示变量名和规则，不显示具体 secret 值。

## 文件和备份

- 新上传文件加密后保存在 `storage/uploads`，不放入 `public`。
...
- 历史明文上传可用 `npm.cmd run documents:migrate-encryption` 迁移；执行前脚本会自动创建备份。
- 备份可用 `backup:create`、`backup:list`、`backup:verify`、`backup:restore`、`backup:prune` 管理，详见 `docs/STAGE15_SECURITY_RELIABILITY.md`。

## 审计与错误日志

- `AuditLog` 记录登录、退出、改密、文件上传/下载/删除、备份、AI 设置、旅行删除和导出等关键操作。
- 审计日志只保存脱敏元数据，不保存密码、完整 API Key、session token、cookie、文件内容或加密密钥。
- 本地结构化错误日志会对常见 secret 字段和 URL query 做脱敏。
