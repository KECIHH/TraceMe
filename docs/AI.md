...
1. 用户填写目的地、出发城市、开始/结束日期、人数、预算、节奏、偏好、必去点和规避项。
2. 服务端调用 AI provider，生成结构化旅行草稿。
3. 草稿保存到 `AiPlanDraft`，状态为 `draft`。
4. 用户预览 Trip、Destination、Place、Itinerary、Checklist、Budget、Route、Note 等内容。
5. 用户可以返回修改输入、重新生成、丢弃草稿或确认创建旅行。
6. 确认后，系统把草稿写入正式旅行数据，并把草稿状态改为 `applied`。
